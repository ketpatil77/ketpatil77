...
mobile_points = " ".join(f"{55+i*(610/(len(weekly)-1)):.1f},{620-(v/peak)*160:.1f}" for i,v in enumerate(weekly))
mobile_area = f"55,620 {mobile_points} 665,620"
mobile_lang = []
for index,(name,value) in enumerate(languages.most_common(5)):
    width=420*value/lang_total
    y=735+index*42
    mobile_lang.append(f'<text x="55" y="{y}" fill="#94a3b8" font-size="14">{name.upper()}</text><rect x="220" y="{y-14}" width="420" height="14" rx="7" fill="#172554"/><rect x="220" y="{y-14}" width="{width:.1f}" height="14" rx="7" fill="{palette[index]}"/><text x="665" y="{y}" fill="#e2e8f0" font-size="13" text-anchor="end">{value/lang_total*100:.1f}%</text>')
mobile_cards=[]
for i,(label,value) in enumerate([("CONTRIBUTIONS / 12M",calendar["totalContributions"]),("PUBLIC REPOSITORIES",repos["totalCount"]),("MERGED PULL REQUESTS",merged),("OPEN PULL REQUESTS",open_prs)]):
    x=55+(i%2)*315; y=105+(i//2)*125
    mobile_cards.append(f'<g transform="translate({x} {y})"><rect width="285" height="96" rx="8" fill="#0f172a" stroke="#1e3a8a"><animate attributeName="stroke-opacity" values=".45;1;.45" dur="3s" repeatCount="indefinite"/></rect><text x="18" y="31" fill="#64748b" font-size="12">{label}</text><text x="18" y="72" fill="#f8fafc" font-size="32" font-weight="700">{value}</text></g>')
mobile=f'''<svg xmlns="http://www.w3.org/2000/svg" width="720" height="980" viewBox="0 0 720 980" role="img" aria-labelledby="mt md"><title id="mt">Mobile GitHub engineering telemetry for {USER}</title><desc id="md">Responsive mobile dashboard with contribution, repository, pull request, and language metrics.</desc><defs><linearGradient id="mb" x2="1" y2="1"><stop stop-color="#050914"/><stop offset="1" stop-color="#0c1831"/></linearGradient><linearGradient id="ma" x2="0" y2="1"><stop stop-color="#3b82f6" stop-opacity=".45"/><stop offset="1" stop-color="#3b82f6" stop-opacity="0"/></linearGradient><filter id="mg"><feGaussianBlur stdDeviation="3" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><rect width="720" height="980" rx="18" fill="url(#mb)"/><g font-family="Consolas,Menlo,monospace"><text x="55" y="57" fill="#f8fafc" font-size="22" font-weight="700">LIVE TELEMETRY / MOBILE</text><text x="665" y="57" fill="#38bdf8" font-size="11" text-anchor="end">SYNC 24H</text>{''.join(mobile_cards)}<text x="55" y="390" fill="#64748b" font-size="13">52-WEEK CONTRIBUTION VELOCITY</text><path d="M55 620H665M55 435V620" stroke="#334155"/><polygon points="{mobile_area}" fill="url(#ma)"/><polyline points="{mobile_points}" fill="none" stroke="#38bdf8" stroke-width="4" filter="url(#mg)" class="trace"/><text x="55" y="655" fill="#64748b" font-size="11">T-52W</text><text x="665" y="655" fill="#64748b" font-size="11" text-anchor="end">NOW / PEAK {peak}</text><text x="55" y="697" fill="#64748b" font-size="13">LANGUAGE SIGNAL</text>{''.join(mobile_lang)}</g><style>.trace{{stroke-dasharray:900;animation:d 3s ease-out both}}@keyframes d{{from{{stroke-dashoffset:900}}to{{stroke-dashoffset:0}}}}@media(prefers-reduced-motion:reduce){{.trace{{animation:none}}}}</style></svg>'''
Path("assets/live-telemetry-mobile.svg").write_text(mobile,encoding="utf-8")
Path("assets/telemetry.json").write_text(json.dumps({
    "contributions": calendar["totalContributions"], "repositories": repos["totalCount"],
    "merged_prs": merged, "open_prs": open_prs, "stars": stars,
    "weekly": weekly, "languages": dict(languages.most_common(8))
